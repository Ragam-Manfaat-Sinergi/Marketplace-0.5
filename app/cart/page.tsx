    ...
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-500 text-sm mt-2">
          Pesananmu akan tampil di sini setelah kamu membuat order.
        </p>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-6">
      <h1 className="text-3xl font-bold mb-8 text-gray-900">
        🛍️ Keranjang & Pesanan Saya
      </h1>

      <div className="grid md:grid-cols-2 gap-6">
        {orders.map((order) => {
          const isPaid = ["paid", "processing", "shipped", "completed"].includes(order.status);
          const isWaiting = ["pending", "awaiting_verification"].includes(order.status);
              ...
              <div className="mt-6 flex justify-end">
                ...
                {isWaiting && (
                  <Link
                    ...
                    className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md shadow-sm text-sm font-medium transition"
                  >
